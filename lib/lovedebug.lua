...
_DebugSettings.Settings = function()
	...
	print(
		"   _DebugSettings.MultipleErrors  [Boolean]  Controls if errors should appear multiple times, default is false"
	)
	print("   _DebugSettings.OverlayColor  [{int, int, int}]  Sets the color of the overlay, default is {0,0,0}")
	print(
		"   _DebugSettings.LiveAuto  [Boolean]  Check if the code should be reloaded when it's modified, default is false"
	)
	print("   _DebugSettings.LiveFile  [String]  Sets the file that lovedebug reloads, default is 'main.lua'")
	print(
		"   _DebugSettings.LiveFile  [{String,String,...}]  Sets the files, has a table, that lovedebug reloads, can be multiple"
	)
	print(
		"   _DebugSettings.LiveReset  [Boolean]  Rather or not love.load() should be reloaded if the code is HotSwapped, default is false"
	)
	print(
		"   _DebugSettings.DrawOnTop  [Boolean]  If the errors and prints should be dispalyed on top of the screen, default is false"
	)
	print(
		"   _DebugSettings.HaltExecution  [Boolean]  Rather or not to halt program execution while console is open, default is true"
	)
	print(
		"   _DebugSettings.AutoScroll  [Boolean]  Rather or not to auto scroll the console once output fills up the console, default is false"
	)
end
	...
	if e then
		...
		for i, v in ipairs(e) do
			love.graphics.print(
				...
				2 + 5 * (i - 1) + (i - 1) * _Debug.Font:getHeight()
			)
		end
	end
	love.graphics.setScissor()
	...
	if #_Debug.Proposals > 0 then
		...
		for i = math.max(_Debug.proposaltoenter - 1, 1), math.min(_Debug.proposaltoenter + 1, #_Debug.Proposals) do
			if i ~= _Debug.proposaltoenter then
				...
				love.graphics.print(
					_Debug.Proposal_String .. _Debug.Proposals[i],
					20 + input_prefix_width - proposal_prefix_width,
					h - 27 - (fontSize - 1) * index
				)
			end
		end
		love.graphics.setColor(255 / 255, 255 / 255, 255 / 255)
...
_Debug.keyConvert = function(key)
			...
			if type(_DebugSettings.LiveFile) == "table" then
				...
				for i = 1, #_DebugSettings.LiveFile do --Setting up lastModified for live files
					if not love.filesystem.exists(_DebugSettings.LiveFile[i]) then --if the file's not found then the live variables are reset
						_Debug.handleError(
							"_DebugSettings.LiveFile: Index "
								.. i
								.. ' file "'
								.. _DebugSettings.LiveFile[i]
								.. '" was not found.'
						)
				...
				if love.filesystem.exists(_DebugSettings.LiveFile) then
					...
					_Debug.handleError(
						'_DebugSettings.LiveFile: File "' .. _DebugSettings.LiveFile .. '" was not found.'
					)
					_DebugSettings.LiveFile = prevfile
					_Debug.liveLastModified = prevmod
				end
	...
	elseif key == "tab" and #_Debug.Proposals > 0 then
		_Debug.input = _Debug.input:sub(1, _Debug.inputMarker)
			.. _Debug.Proposals[_Debug.proposaltoenter]
			.. _Debug.input:sub(_Debug.inputMarker + 1)
...
_Debug.handleVirtualKey = function(a)
	if type(a) == "string" then
		...
		if _Debug.resetProposals then
			if
				_Debug.inputMarker == 0
				or _Debug.input:sub(_Debug.inputMarker + 1, _Debug.inputMarker + 1):find("[0-9a-zA-Z_]")
			then
				_Debug.ProposalLocation = _G
				_Debug.Proposal_String = ""
			else
	...
	return function()
		while true do
			...
			if love.update and not _Debug.drawOverlay then
				if
					_DebugSettings.LiveAuto
					and _Debug.liveCheckLastModified(_DebugSettings.LiveFile, _Debug.liveLastModified)
				then
					if type(_DebugSettings.LiveFile) == "table" then
						for i = 1, #_DebugSettings.LiveFile do
							if
								love.filesystem.getInfo(_DebugSettings.LiveFile[i]).modtime
								~= _Debug.liveLastModified[i]
							then
				...
				if type(_DebugSettings.LiveFile) == "table" then
					for i = 1, #_DebugSettings.LiveFile do
						if
							(
								_DebugSettings.LiveAuto
								and love.filesystem.getInfo(_DebugSettings.LiveFile[i])
									~= _Debug.liveLastModified[i]
							) or _Debug.liveDo
						then
							_Debug.hotSwapUpdate(dt, _DebugSettings.LiveFile[i])
